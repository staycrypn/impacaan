...
if 'current_card' not in st.session_state:
    st.session_state.current_card = random.choice(FULL_DECK)
...
# Function to render the card visually
def render_card(code):
    rank = code[:-1]
    suit = SUIT_SYMBOLS[code[-1]]
    color = "red" if suit in ["♡","♦"] else "black"
    
    # Green border only if magician-signal
...
# Buttons keeping exact Tkinter logic
